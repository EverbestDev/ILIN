...
import { signInWithEmailAndPassword, linkWithCredential } from '../../utility/firebase';
...
describe('Login - account exists flow', () => {
  ...
  it('shows provider banner when account exists with google as provider', async () => {
    ...
    const facebookBtn = screen.getAllByRole('button').find((b) => b.querySelector('svg')); // heuristic
    ...
    const fbButton = screen.getByRole('button', { name: /Sign in with Facebook/i });
    ...
    expect(screen.getByText(/Sign in with Google to link|Sign in with Google/i)).toBeInTheDocument();
  });
  ...
  it('prompts for email when provider error has no email', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Enter the email to check which sign-in methods/i)).toBeInTheDocument();
    });
  });
  ...
  it('resolves and links when password is provided', async () => {
    ...
    signInWithEmailAndPassword.mockResolvedValueOnce({ user: { getIdToken: async () => 'token' } });
    ...
    await waitFor(() => {
      expect(signInWithEmailAndPassword).toHaveBeenCalledWith(expect.anything(), 'testpass@example.com', 'correcthorsebatterystaple');
