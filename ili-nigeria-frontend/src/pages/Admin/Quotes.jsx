...
export default function AdminQuotes() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `ilin_quotes_${new Date().toISOString().split("T")[0]}.csv`
    );
  ...
  const stats = {
    ...
    thisWeek: quotes.filter((q) => {
      const diffDays = Math.floor(
        (new Date() - new Date(q.createdAt)) / (1000 * 60 * 60 * 24)
      );
      return diffDays <= 7;
    }).length,
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="mt-1 text-slate-600">
              Breaking language barriers, connecting cultures worldwide
            </p>
          </div>
        </div>
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-100">
                    Complete details and documents
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {selectedQuote.targetLanguages?.map((lang, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-1 text-xs font-medium text-purple-800 bg-purple-200 rounded-full"
                        >
                          {lang}
                        </span>
                      ))}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="p-5 border border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100/50 rounded-xl">
                  ...
                  <span
                    className={`inline-flex px-4 py-2 text-sm font-medium rounded-full border ${getUrgencyColor(
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-relaxed text-blue-100">
                      At ILIN, we're dedicated to breaking language barriers and
                      fostering global understanding. Every translation request
                      represents a bridge we're building between cultures,
                      enabling meaningful communication and collaboration across
                      borders.
                    </p>
                  </div>
                </div>
